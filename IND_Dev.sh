...
if [ "$who" != "root" ]; then
	tmp1=$(grep IND /home/pi-star/.bashrc)
#	echo "1:$tmp1"
	if [ -z "$tmp1" ]; then
		sudo echo "alias IND='sudo /home/pi-star/Scripts/IND_Dev.sh'" >> /home/pi-star/.bashrc
	fi
  	sudo su
else
  	echo "I AM GROOT - OK to Proceed!"
	tmp2=$(grep IND /root/.bashrc)
	if [ -z "$tmp2" ]; then
		sudo echo "alias IND='sudo /home/pi-star/Scripts/IND_Dev.sh'" >> /root/.bashrc
	fi
